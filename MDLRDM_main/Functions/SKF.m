...
for i = 1 : C
    newW1{i} = Wall{i}./repmat(sum(Wall{i},1),n,1);%按列做归一化
end
sumW1 = zeros(m,n);
for i = 1 : C
    sumW1= sumW1 + newW1{i};%%将归一化后的几个相似图 累加起来
end


for i = 1 : C
    Wall{i} = Wall{i}./repmat(sum(Wall{i},1),n,1);%按列做归一化
end
...
for ITER=1:t
    for i = 1 : C           
         Wall{i}=ALPHA*newW{i}*(Wsum - Wall{i})*newW{i}'/(C-1) + (1-ALPHA)*(sumW1 - newW1{i})/(C-1);%融合主要步骤
    end   
    Wsum = zeros(m,n);
    for i = 1 : C
        Wsum = Wsum + Wall{i};
...
for ii=1:mm	%mm是行
	for jj=ii:mm%jj是列，上三角矩阵，
		iu = similar_m(ii,:);
		iu_list = sort(iu,'descend');%列不会小于行的下标，从而限制取到两个相互样本的k的最近邻居
